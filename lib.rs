...
mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn admin_move_market_to_closed(
            ...
            Ok((
                Some(T::WeightInfo::admin_move_market_to_closed(close_ids_len)),
                Pays::No,
            )
                .into())
        }
        ...
        pub fn dispute(
            ...
            let dispute_mechanism = market
                .dispute_mechanism
                .as_ref()
                .ok_or(Error::<T>::NoDisputeMechanism)?;
        ...
        pub fn edit_market(
            ...
            let old_market = <zrml_market_commons::Pallet<T>>::market(&market_id)?;
                ...
                old_market.status == MarketStatus::Proposed,
                Error::<T>::InvalidMarketStatus
            );
        ...
        pub fn redeem_shares(
            ...
            let market_account = Self::market_account(market_id);
                ...
                Error::<T>::MarketIsNotResolved
            );
            ensure!(
                market.is_redeemable(),
                Error::<T>::InvalidResolutionMechanism
            );

            // Check to see if the sender has any winning shares.
            let resolved_outcome = market
                .resolved_outcome
                .ok_or(Error::<T>::MarketIsNotResolved)?;
            ...
            let winning_assets = match resolved_outcome {
                ...
                OutcomeReport::Scalar(value) => {
                    ...
                    let calc_payouts =
                        |final_value: u128, low: u128, high: u128| -> (Perbill, Perbill) {
                            if final_value <= low {
        ...
        pub fn reject_market(
            ...
            Ok((
                Some(T::WeightInfo::reject_market(
                    close_ids_len,
                    reject_reason_len,
                )),
                Pays::No,
        ...
        pub fn start_global_dispute(
            ...
            let dispute_mechanism = market
                ...
                matches!(
                    market.status,
                    MarketStatus::Disputed | MarketStatus::Reported
                ),
                Error::<T>::InvalidMarketStatus
            );
            ...
            let report = market
                .report
                .as_ref()
                .ok_or(Error::<T>::MarketIsNotReported)?;
        ...
        pub fn create_market_and_deploy_pool(
            ...
            Ok(Some(T::WeightInfo::create_market_and_deploy_pool(
                ids_len,
                spot_prices_len,
            ))
            .into())
        ...
        pub fn schedule_early_close(
            ...
            let new_period = if let Some(p) = &market.early_close {
                    ...
                    is_authorized,
                    Error::<T>::OnlyAuthorizedCanScheduleEarlyClose
                );
                ...
                let (block_period, time_frame_period) = if is_authorized {
                    ...
                    (
                        T::CloseEarlyBlockPeriod::get(),
                        T::CloseEarlyTimeFramePeriod::get(),
                    )
                };
            ...
            Self::deposit_event(Event::MarketEarlyCloseScheduled {
                market_id,
                new_period,
                state,
            });
        ...
        pub fn dispute_early_close(
            ...
            let market = <zrml_market_commons::Pallet<T>>::market(&market_id)?;
            Self::ensure_market_is_active(&market)?;
            let mut early_close = market
                .early_close
                .ok_or(Error::<T>::NoEarlyCloseScheduled)?;
        ...
        pub fn close_trusted_market(
            ...
            let market = <zrml_market_commons::Pallet<T>>::market(&market_id)?;
                ...
                market.dispute_mechanism.is_none(),
                Error::<T>::MarketIsNotTrusted
            );
            Self::ensure_market_is_active(&market)?;
            let close_ids_len = Self::clear_auto_close(&market_id)?;
            Self::close_market(&market_id)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn clear_auto_resolve(market_id: &MarketIdOf<T>) -> Result<(u32, u32), DispatchError> {
            ...
            let (ids_len, mdm_len) = match market.status {
                ...
                MarketStatus::Disputed => {
                    ...
                    let ResultWithWeightInfo {
                        result: auto_resolve_block_opt,
                        weight: _,
        ...
        pub(crate) fn calculate_time_frame_of_moment(time: MomentOf<T>) -> TimeFrame {
            time.saturated_into::<TimeFrame>()
                .saturating_div(MILLISECS_PER_BLOCK.into())
        }
            ...
            <zrml_market_commons::Pallet<T>>::mutate_market(&market_id, |market| {
                ...
                match market.period {
                    ...
                    MarketPeriod::Timestamp(ref range) => {
                        ...
                        let oracle_duration_in_moments: MomentOf<T> = market
                            .deadlines
                            .oracle_duration
                            .saturated_into::<u32>()
                            .into();
