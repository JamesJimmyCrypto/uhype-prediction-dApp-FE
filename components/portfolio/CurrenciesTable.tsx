...
const MoveButton = ({
  ...
  return (
    <>
      {<TransferButton assetId={transferAssetId} />}
      {!isNativeTokenBalance && sourceChain && destinationAsset != null && (
        <WithdrawButton
          toChain={sourceChain}
          tokenSymbol={token}
          balance={balance}
...
const CurrenciesTable = ({ address }: { address: string }) => {
  ...
  const tableData: TableData[] | undefined = useMemo(() => {
    return sorting
      ...
      .filter(isNotNull)
      .map((balance) => {
        const amount = convertDecimals(balance.balance, balance.decimals, 10);
        return {
          chain: (
            <ImageAndText
              name={balance.chain}
